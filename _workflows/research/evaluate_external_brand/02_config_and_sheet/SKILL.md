---
name: evaluate_external_brand_02_config_and_sheet
description: >-
  Step 02 for evaluate_external_brand: resolve the workspace display name, the workspace's
...
# Evaluate external brand — 02 Config and sheet
...
- Google OAuth for Drive + Sheets: `{workspace_root}/credentials.json` at `google.oauth_token_unified`. Scopes: `drive` and `spreadsheets`.
...
Resolve `workspace_name` per [Resolve workspace display name](../SKILL.md#resolve-workspace-display-name): caller input → `{workspace_root}/config.json` `workspace_name` → inferred from domain/folder. If it was inferred, show the user the resolved name and the exact two queries and confirm before any search happens in step 03.

## 3. Resolve workspace domains (exclusion set)

These are the hosts that count as "the workspace's own domain" and are dropped from the SERP in step 03.
...
4. Build `workspace_hosts` = the deduped apex host set. Treat both the apex and any `www.` subdomain as the workspace when matching in step 03 (match on apex so subdomains like `blog.`, `docs.`, `www.` are all excluded).
...
```text
[run-debug] workflow=_workflows/evaluate_external_brand | DOMAINS | workspace_hosts=[...]
```
...
```text
[run-debug] workflow=_workflows/evaluate_external_brand | SHEET | created spreadsheet_id={id} folder={drive_folder_id} tab="{tab_title}"
```
...
Next: [../03_serp_collection/SKILL.md](../03_serp_collection/SKILL.md)
