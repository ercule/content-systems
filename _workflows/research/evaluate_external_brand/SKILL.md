---
name: evaluate_external_brand
description: >-
  Shared external brand evaluator. For a given workspace, runs the bare-brand SERPs
...
```text
[run-debug] workflow=_workflows/evaluate_external_brand | <PHASE> | <facts>
```
...
- `workspace_domains` — optional. One or more apex hosts that count as "the workspace's domain" and are excluded from results (e.g. `acme.com`). If omitted, resolve from `{workspace_root}/config.json` per step 02.
...
- `Mention` — the single paragraph from that page that talks about the brand, as extracted in step 04. One page per row.

## Mention quality rules

These apply to every `Mention` before it is written, in step 04:
...
1. [01_preflight/SKILL.md](01_preflight/SKILL.md) — verify config, credentials, and Drive folder.
2. [02_config_and_sheet/SKILL.md](02_config_and_sheet/SKILL.md) — resolve `workspace_name`, workspace domains, and Drive folder; create the new `Site | Mention` sheet in that folder.
3. [03_serp_collection/SKILL.md](03_serp_collection/SKILL.md) — run the two brand SERPs (up to position `max_results`), merge, dedupe, and drop owned-domain results to produce `external_results`.
4. [04_extract_and_append/SKILL.md](04_extract_and_append/SKILL.md) — render each external page with local Playwright, extract the relevant paragraph, and append each `Site | Mention` row to the sheet as soon as it is found.
...
```text
[run-debug] workflow=_workflows/evaluate_external_brand | START | workspace={workspace_slug} workspace_name="{workspace_name}"
[run-debug] workflow=_workflows/evaluate_external_brand | SHEET | created spreadsheet_id={id} folder={folder_id}
[run-debug] workflow=_workflows/evaluate_external_brand | SERP | query="..." results=N kept=N dropped_owned=N
...
[run-debug] workflow=_workflows/evaluate_external_brand | EXTRACT | url=... mention_chars=N
...
[run-debug] workflow=_workflows/evaluate_external_brand | DONE | rows=N pages_failed=N needs_manual_scrape=N sheet={url}
```
...
- [ ] [02_config_and_sheet/SKILL.md](02_config_and_sheet/SKILL.md) completed (new sheet created)
- [ ] [03_serp_collection/SKILL.md](03_serp_collection/SKILL.md) completed
- [ ] [04_extract_and_append/SKILL.md](04_extract_and_append/SKILL.md) completed
