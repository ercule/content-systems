...
description: >-
  ...
  separate faq-question-responder workflow.
"last updated": 2026-06-07T20:26:00+00:00
"last run": 2026-06-27
---

# FAQ Question Finder

Step 0: Read [setup/run_workflow/SKILL.md](../../../setup/run_workflow/SKILL.md) (runtime HTTP, logging, ephemeral runs). Build calls at request time and do not depend on the exported n8n workflow IDs.
...
This shared workflow finds candidate questions from Reddit, LinkedIn, and Google Search Console. It appends only `Topic | Question | Source` rows to `FAQ Coverage`. The [faq-question-responder](../faq_question_responder/SKILL.md) workflow fills `Response Page` and `Draft Response` later.

Log line prefix:

```text
[run-debug] workflow=_workflows/faq_question_finder | <PHASE> | <facts>
```
...
- Optional GSC fallback account: `google.oauth_token_unified_2`, if present. Step 03 falls back to it only when the primary account cannot read a matching Search Console property. Generate it with the standard Google OAuth installed-app flow (Desktop client in `google.oauth_installed_client`).
...
Leave every other column (e.g. `LLM Response`, `Draft Response`, `Response Page`, `Mentioned`, `Topics`, `% Mentioned`) untouched. The `Response Page` and `Draft Response` columns are filled by the separate [faq-question-responder](../faq_question_responder/SKILL.md) workflow.

## Question Quality Rules

These rules apply to every question before it is written to `FAQ Coverage`, regardless of source (Reddit, LinkedIn, or GSC). Steps 03 and 04 both enforce them.
...
1. [01-sheet-and-topics/SKILL.md](01_sheet_and_topics/SKILL.md) - resolve the sheet, validate headers, and build the topic queue plus dedupe sets.
2. [02-source-collection/SKILL.md](02_source_collection/SKILL.md) - collect Reddit and LinkedIn source text.
3. [03-gsc-questions/SKILL.md](03_gsc_questions/SKILL.md) - resolve the GSC property and mine GSC question candidates matched to Strategy topics.
4. [04-extract-and-append/SKILL.md](04_extract_and_append/SKILL.md) - extract questions, filter them, and append each `Topic | Question | Source` row to `FAQ Coverage` as soon as it is found.

Append rows incrementally as results are found. Do not hold questions in memory and write them only at the end of the run.

The end state is appended rows in `FAQ Coverage` with `Topic`, `Question`, and `Source` populated. No website pages are edited. To fill `Response Page` and `Draft Response` on those rows, run the [faq-question-responder](../faq_question_responder/SKILL.md) workflow next.

## Logging Template

```text
[run-debug] workflow=_workflows/faq_question_finder | START | sheet={spreadsheet_id}
[run-debug] workflow=_workflows/faq_question_finder | SHEET | strategy="{strategy_title}" faq="{faq_title}"
[run-debug] workflow=_workflows/faq_question_finder | QUEUE | topics=N max={max_new_topics}
[run-debug] workflow=_workflows/faq_question_finder | SERP | topic="..." source=reddit results=N kept=N
[run-debug] workflow=_workflows/faq_question_finder | SCRAPE | source=reddit url=... status=... chars=N
[run-debug] workflow=_workflows/faq_question_finder | EXTRACT | topic="..." source=reddit questions=N
[run-debug] workflow=_workflows/faq_question_finder | GSC | window={start}..{end} rows_fetched=N kept_questions=N
...
- [ ] [01-sheet-and-topics/SKILL.md](01_sheet_and_topics/SKILL.md) completed
- [ ] [02-source-collection/SKILL.md](02_source_collection/SKILL.md) completed
- [ ] [03-gsc-questions/SKILL.md](03_gsc_questions/SKILL.md) completed
- [ ] [04-extract-and-append/SKILL.md](04_extract_and_append/SKILL.md) completed
